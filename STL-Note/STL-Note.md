...
在实际的SGI STL源代码中临时迭代器标签类对象的创建并不是按照我上面所写的那样，因为这种方式需要对每一个算法都做一遍typedef或者using。重复的东西应该从中剥离处理，独立成函数模块，以避免冗余。这种情况不仅是针对iterator_category而言，别的迭代器相关类型也可能具有这种需求。所以SGI STL在为迭代器定义了如下几个辅助函数，以方便算法内部调用的实现函数可以知道迭代器相关的类型信息（通过传递不同类型的对象，以在编译期就能知晓，辅助重载函数解析）：
